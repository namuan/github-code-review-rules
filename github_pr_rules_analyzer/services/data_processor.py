...
from ..models import ReviewComment, CodeSnippet, CommentThread, ExtractedRule, RuleStatistics
...
class DataProcessor:
    ...
    def add_review_comment_task(self, comment_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"Error adding review comment task: {e}")
            return False
    ...
    def add_code_snippet_task(self, snippet_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"Error adding code snippet task: {e}")
            return False
    ...
    def add_comment_thread_task(self, thread_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"Error adding comment thread task: {e}")
            return False
    ...
    def add_rule_extraction_task(self, rule_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"Error adding rule extraction task: {e}")
            return False
    ...
    def _process_review_comment(self, comment_data: Dict[str, Any]):
        """
    ...
    def _process_code_snippet(self, snippet_data: Dict[str, Any]):
        """
        ...
        try:
            ...
            logger.error(f"Error processing code snippet: {e}")
            raise
    
    def _process_comment_thread(self, thread_data: Dict[str, Any]):
        """
        ...
        try:
            ...
            logger.error(f"Error processing comment thread: {e}")
            raise
    
    def _extract_rule(self, rule_data: Dict[str, Any]):
        """
    ...
    def _update_statistics(self, stats_data: Dict[str, Any]):
        """
        ...
        try:
            ...
            existing_stats = self.session.query(RuleStatistics).filter(
    ...
    def _validate_code_snippet(self, snippet_data: Dict[str, Any]) -> bool:
        ...
        if (snippet_data['line_start'] <= 0 or 
            snippet_data['line_end'] <= 0 or 
            snippet_data['line_start'] > snippet_data['line_end']):
            return False
    ...
    def _validate_comment_thread(self, thread_data: Dict[str, Any]) -> bool:
        ...
        for field in required_fields:
            if field not in thread_data or not thread_data[field]:
                return False
    ...
    def _upsert_review_comment(self, comment_data: Dict[str, Any]) -> ReviewComment:
        ...
        existing_comment = self.session.query(ReviewComment).filter(
    ...
    def _upsert_code_snippet(self, snippet_data: Dict[str, Any]):
        """
        Create or update code snippet
        ...
        existing_snippet = self.session.query(CodeSnippet).filter(
        ...
        if existing_snippet:
            ...
            review_comment = self.session.query(ReviewComment).filter(
    ...
    def _upsert_comment_thread(self, thread_data: Dict[str, Any]):
        """
        Create or update comment thread
        ...
        existing_thread = self.session.query(CommentThread).filter(
        ...
        if existing_thread:
            ...
            # Create new thread
            from ..models import ReviewComment, PullRequest
            review_comment = self.session.query(ReviewComment).filter(
    ...
    def _extract_rule_from_text(self, text: str) -> Optional[str]:
        """
        ...
        for sentence in sentences:
            ...
            if sentence and len(sentence) > 10:  # Reasonable length
                ...
                imperative_verbs = [
                    'use', 'avoid', 'follow', 'ensure', 'make', 'remember', 'do', 'always', 'never',
                    'prefer', 'implement', 'add', 'remove', 'change', 'update', 'fix', 'refactor',
                    'optimize', 'simplify', 'standardize', 'document', 'test', 'validate'
                ]
