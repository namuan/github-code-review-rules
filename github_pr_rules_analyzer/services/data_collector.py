...
from ..models import Repository, PullRequest, ReviewComment, CodeSnippet, CommentThread
...
class DataCollector:
    ...
    def collect_repository_data(self, owner: str, repo: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.github_client.validate_repository_access(owner, repo):
                raise Exception(f"Cannot access repository {owner}/{repo}")
            ...
            for pr_data in closed_prs:
                try:
                    ...
                    error_msg = f"Error collecting PR #{pr_data['number']}: {str(e)}"
            ...
            logger.info(f"Collected: {len(results['pull_requests'])} PRs, "
            ...
            error_msg = f"Error collecting repository data: {str(e)}"
            logger.error(error_msg)
            results['errors'].append(error_msg)
            results['end_time'] = datetime.utcnow()
            return results
    ...
    def _upsert_repository(self, repo_data: Dict[str, Any]) -> Repository:
        ...
        existing_repo = self.session.query(Repository).filter(
        ...
        try:
            ...
            for comment_data in all_comments:
                try:
                    ...
                    error_msg = f"Error processing comment {comment_data.get('id', 'unknown')}: {str(e)}"
                    logger.error(error_msg)
                    results['errors'].append(error_msg)
    ...
    def _upsert_pull_request(self, pr_data: Dict[str, Any], repository_id: int) -> PullRequest:
        ...
        existing_pr = self.session.query(PullRequest).filter(
    ...
    def _create_comment_thread(self, review_comment: ReviewComment, pull_request_id: int) -> Optional[CommentThread]:
        """
        ...
        try:
            ...
            logger.error(f"Error creating comment thread: {str(e)}")
            return None
    ...
    def cleanup_old_data(self, days: int = 30) -> Dict[str, int]:
        ...
        try:
            ...
            deleted_snippets = self.session.query(CodeSnippet).filter(
                CodeSnippet.created_at < cutoff_date
            ).delete()
            results['code_snippets'] = deleted_snippets
            
            # Delete old comment threads
            deleted_threads = self.session.query(CommentThread).filter(
            ...
            deleted_comments = self.session.query(ReviewComment).filter(
