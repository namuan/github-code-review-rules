...
class PullRequest(Base):
    ...
    # Indexes
    __table_args__ = (
        Index('idx_pull_requests_dates', 'created_at', 'closed_at'),
    )
    ...
    def from_github_data(cls, github_data, repository_id):
        ...
        return cls(
            ...
            closed_at=datetime.fromisoformat(github_data["closed_at"].replace("Z", "+00:00")) if github_data.get("closed_at") else None,
            merged_at=datetime.fromisoformat(github_data["merged_at"].replace("Z", "+00:00")) if github_data.get("merged_at") else None,
    ...
    def update_from_github_data(self, github_data):
        ...
        self.created_at = datetime.fromisoformat(github_data["created_at"].replace("Z", "+00:00")) if github_data.get("created_at") else None
        self.closed_at = datetime.fromisoformat(github_data["closed_at"].replace("Z", "+00:00")) if github_data.get("closed_at") else None
        self.merged_at = datetime.fromisoformat(github_data["merged_at"].replace("Z", "+00:00")) if github_data.get("merged_at") else None
