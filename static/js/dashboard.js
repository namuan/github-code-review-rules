                    ...
                    callbacks: {
                        label: function(context) {
                            ...
                            if (context.parsed !== null) {
                                label += new Intl.NumberFormat('en-US', { style: 'percent' }).format(context.parsed);
                            }
                            return label;
                        }
                    }
...
function fetchRulesList() {
    ...
    // Build API URL
    const apiUrl = `/api/v1/rules/search?query=${encodeURIComponent(searchQuery)}&category=${encodeURIComponent(categoryFilter)}&severity=${encodeURIComponent(severityFilter)}&repository=${encodeURIComponent(repositoryFilter)}`;
...
function initEventListeners() {
    ...
    // Stop sync
    document.getElementById('stop-sync-btn').addEventListener('click', function () {
