...
function fetchRulesList() {
  ...
  // Build API URL
  const apiUrl = `/api/v1/rules/search?query=${encodeURIComponent(
    searchQuery,
  )}&category=${encodeURIComponent(
    categoryFilter,
  )}&severity=${encodeURIComponent(
    severityFilter,
  )}&repository=${encodeURIComponent(repositoryFilter)}`;
  ...
  data.rules.forEach((rule) => {
    ...
    ruleElement.innerHTML = `
            <div class="rule-header">
                ...
                <div class="rule-meta">
                    <span class="category rule-tag category">${
                      rule.rule_category
                    }</span>
                    <span class="severity rule-tag severity">${
                      rule.rule_severity
                    }</span>
                </div>
            </div>
...
function updatePagination(data) {
  ...
  pageInfo.textContent = `Showing ${data.skip + 1} to ${
...
function initEventListeners() {
  ...
  document
    .getElementById("sync-selected-btn")
    .addEventListener("click", function () {
